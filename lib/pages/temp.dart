...
Widget _buildAnalysisResult() {
  if (_analysisData == "ddd") {
    return Text("No data available");
  }

  final analysis = jsonDecode(_analysisData);
  if (!analysis['exists']) {
  ...
  final cropName = firstResult['cropName'];
  final status = firstResult['status'];
  final diseaseImages = firstResult['diseaseImages'];
  final remedies = firstResult['remedies'];
  ...
  return Column(
    ...
    children: [
      ...
      SizedBox(height: 16),
      Text("Disease Images:", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
      SizedBox(height: 8),
      diseaseImages != null
          ? Wrap(
              spacing: 8.0,
              runSpacing: 8.0,
              children: List<Widget>.generate(diseaseImages.length, (index) {
          ...
          : Text("No disease images available."),
      SizedBox(height: 16),
      Text("Remedies:", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
      SizedBox(height: 8),
      remedies != null
          ? Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: remedies.map<Widget>((remedy) => Text("- $remedy")).toList(),
            )
          : Text("No remedies available."),
    ],
  );
}
