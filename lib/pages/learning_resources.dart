...
class _LearningResourcesState extends State<LearningResources> {
  ...
  Widget build(BuildContext context) {
    List<dynamic> filteredCourses = courseMap.values.where((course) {
      final matchesCategory = selectedCategories.isEmpty || selectedCategories.contains(course['domain']);
      final matchesQuery = course['title'].toLowerCase().contains(query.toLowerCase());
  ...
  Widget courseCard(String id, BuildContext context, Map<String, dynamic> courseData) {
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => CoursesPlayer(coursetitle: courseData['title'])),
        );
      },
      child: Container(
